...
func fixture() (*wire.MsgTx, []byte, []byte) {
	...
	tx := wire.NewMsgTx()
	tx.Version = 2
	tx.AddTxIn(wire.NewTxIn(&wire.OutPoint{}, 20000, nil))
	...
	sig, _ := t.NewScriptBuilder().AddData(witness).AddData(prefix).AddData(redeem).Script()
